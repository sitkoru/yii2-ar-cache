...
local objChanged = cjson.decode(ARGV[3])
...
local conj_cache_key_wild = function(db_table, scheme, obj, objChanged)
    local parts = {}
    local first = true
    for field in string.gmatch(scheme, "[^,]+") do
        if (objChanged[field] ~= nil and first) then
            table.insert(parts, field .. '=*')
        end
        if (obj[field] ~= nil and first == false) then
            table.insert(parts, field .. '=' .. tostring(obj[field]))
        end
        first = false
    end
...
for _, scheme in ipairs(schemes) do
    ...
    table.insert(conj_keys, conj_cache_key_wild(db_table, scheme, obj, objChanged))
end
